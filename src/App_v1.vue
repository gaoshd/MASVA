<script setup lang="ts">
    ...
    function drawEpisodeView(value_names){
        ...
        d3.json(`./football_data_v2/episode_detail_json/episode_${curren_episode_id.value}.json`).then((result)=> {
            ...
            function drawActionRidgelineView(){
                ...
                const action_names = ['abdomen', 'right_hip', 'right_knee', 'left_hip', 'left_knee', 'right_shoulder1', 'right_shoulder2', 'right_elbow', 'left_shoulder1', 'left_shoulder2', 'left_elbow']
                ...
                const overlap = 2.8;
                const a_width = 400;
                const a_height = series.length * 30;
                const marginTop = 30;
                ...
                const actions_zScale = d3.scaleLinear()
                    .domain(d3.extent(d3.merge(flatData).sort(d3.ascending))).nice()
                ...
                function action_group_event(){
                    ...
                    if(d3.event.type == "mouseout" ? false:true){
                        ...
                        step: ${mouse_step}`)
                            ...
                            .style("top", (d3.event.pageY - 28) + "px")
                            .style("display", "block")
                        current_action_group.attr("opacity", 1)
                    } else {
                        tooltip.style("display", "none");
                        current_action_group.attr("opacity", 0.2)
                    }
                }
    ...
    function actionPictogramView() {
        const ap_width = 400, ap_height = 400, ap_margin = 20;
        ...
        const joint_data = [
            ...
            {name: 'shoulder1', x: head_pos.x - 60, y: head_pos.y + 40, r: joint_r},
            {name: 'shoulder2', x: head_pos.x + 60, y: head_pos.y + 40, r: joint_r},
        ];
        const body_data = [
            {name: 'head', x: head_pos.x, y: head_pos.y, r: 20},
            {name: 'shoulder1', x: head_pos.x - 60, y: head_pos.y + 40, r: joint_r},
            {name: 'shoulder2', x: head_pos.x + 60, y: head_pos.y + 40, r: joint_r},
        ];
