        ...
        d3.json(`./football_data_v2/episode_detail_json/episode_${curren_episode_id.value}.json`).then((result)=> {
            ...
            function drawActionRidgelineView(){
                ...
                const body_flatData = step_data.map(d => {
                    let body_indicators = []
                    Object.entries(d.body_info).forEach(d1 => {
                        body_indicators.push(d1[1][current_body_indicator.value])
                    })
                    return body_indicators
                });

                const action_names = ['torso', 'lwaist', 'pelvis', 'right_thigh', 'right_shin', 'right_foot', 'left_thigh', 'left_shin', 'left_foot', 'right_upper_arm', 'right_lower_arm', 'left_upper_arm', 'left_lower_arm']

                const series = d3.transpose(body_flatData).map((arr,i) => {
                ...
                const actions_zScale = d3.scaleLinear()
                    .domain(body_indicators_extent).nice()
                ...
                const group = actions_svg.append("g")
                    ...
                    .on("click", action_group_click)
                    .append("path")
                    .attr("fill", "none")
                    .attr("d", d => line(d.values));

                function action_group_click() {
                    let mouse_step = step_xScale.invert(d3.mouse(this)[0]).toFixed(0)
                    let stepline_svg = d3.select("#stepLineView svg")
                    let step_circle_node = stepline_svg.select(`circle.step_${mouse_step}`)
                ...
                const tip_line = actions_svg.append("line").attr("class", "tip_line").attr("x1", step_xScale(curren_step_id.value)).attr("y1", 0)
                                        ...
                                        .attr("stroke-width", 2)
                                        .on("mouseover", tip_line_event)
                                        .on("mouseout", tip_line_event)

                function tip_line_event() {
                    const tooltip = d3.select('div.tooltip_container')
                    if(d3.event.type == "mouseout" ? false:true){
                        d3.select(this).attr("stroke-width", 3)
                        tooltip.style("opacity", .9);
                        tooltip.html(`step: ${curren_step_id.value}<br/>`)
                            .style("left", (d3.event.pageX + 10) + "px")
                ...
                function action_group_event(){
                    ...
                    let mouse_step = step_xScale.invert(d3.mouse(this)[0]).toFixed(0)
                    let mouse_step_value = current_action_group.data()[0].values[mouse_step]
                    if(d3.event.type == "mouseout" ? false:true){
                        tooltip.style("opacity", .9);
                        tooltip.html(`body_name: ${body_name}<br/>
                        step: ${mouse_step}<br/>
                        ${current_body_indicator.value}: ${mouse_step_value}`)
                            .style("left", (d3.event.pageX + 10) + "px")
                            .style("top", (d3.event.pageY + 20) + "px")
                            .style("display", "block")
