<script setup lang="ts">
        ...
        d3.json(`./football_data_v2/episode_detail_json/episode_${curren_episode_id.value}.json`).then((result)=> {
            ...
            function drawEpisodeValueImage(){
                ...
                function agent_event() {
                    ...
                    const tipinfo = `${agent_name}_vel_x: ${agent_data.agent_vel[0]}<br/>
                    ${agent_name}_vel_y: ${agent_data.agent_vel[1]}<br/>
                    ${agent_name}_vel_z: ${agent_data.agent_vel[2]}<br/>
                    ${agent_name}_pos_x: ${agent_data.agent_pos[0]}<br/>
                    ${agent_name}_pos_y: ${agent_data.agent_pos[1]}<br/>
                    ${agent_name}_pos_z: ${agent_data.agent_pos[2]}<br/>`
                    if(d3.event.type == "mouseout" ? false:true){
                        tooltip.style("opacity", .9);
                ...
                function updateValueImage() {
                    if (curren_step_id.value +1 == step_len){
                        return
                    }
                    curren_step_id.value = curren_step_id.value + 1
                    const startValue = curren_step_id.value - 9 < 0 ? 0 : curren_step_id.value - 9;
                    const endStepsValue = (curren_step_id.value + 10) >= step_len ? step_len: curren_step_id.value + 10
                    current_steps.value = d3.range(startValue, endStepsValue)
                    drawParallerLine()
            ...
            function drawActionRidgelineView(){
                ...
                const step_data = episode_data[curren_main_agent.value].step_data
                const step_len = step_data.length
                const w  = 1100, h = 300, m = 50;
                const width = w - 2 * m, height = h - 2 * m;
                const step_actions_data = step_data.map(d => d.actions)
                const all_actions_data = d3.merge(step_actions_data).sort(d3.acending)
                const normalize_scale = d3.scaleLinear().range([0, 1]).domain(d3.extent(all_actions_data));
                const prob_data = step_actions_data.map(d => {
                    let new_data = []
                    d.forEach(ele => {
                        new_data.push(normalize_scale(ele))
                    })
                    return new_data
                })
                ...
                const stack = d3.stack()
                    .keys(d3.range(0, 11)).order(d3.stackOrderNone).offset(d3.stackOffsetExpand)

                const actionColorScale = d3.scaleOrdinal(d3.schemeSpectral[11]).domain(d3.range(0, 11));

                const yScale = d3.scaleLinear().range([height, 0]).domain([0,1]);

                const area_k = (width) / step_len
                const xScale = d3.scaleLinear().range([0, step_len * area_k]).domain([0, step_len - 1]);

                const xAxis = d3.axisBottom(xScale).ticks(Math.round(step_len/8));
                const yAxis = d3.axisLeft(yScale);

                const svg = d3.select("#actionRidgelineView")
                        .append("svg")
                        .attr("height", h)
                        .attr("width", w)
                ...
                drawProbBackGround()
                function drawProbBackGround() {
                    const stackedData = stack(prob_data);
                    console.log(stackedData)
                    const actionChart =  svg.append("g").attr("transform", `translate(${m}, ${m})`)
                    const xG = actionChart.append("g").attr("class", "x-axis")
                        .attr("transform", `translate(${[0, height]})`).call(xAxis);
                    const yG = actionChart.append("g").attr("class", "y-axis").call(yAxis);

                    const pathG = actionChart.append("g")
                    pathG.selectAll("path")
                            .data(stackedData)
                            .enter()
                            .append("path")
                            .attr("d", area)
                            .style("fill", (d, i) => actionColorScale(i))
                            .style("opacity", 0.8)
                            .attr("stroke", "black")
                            .attr("stroke-width", 0)
                            .on("mouseover", actionPathEvent)
                            .on("mouseout", actionPathEvent)

                    function actionPathEvent(){
                        if(d3.event.type == "mouseout" ? false:true){
                            d3.select(this).style("opacity", 1).attr("stroke-width", 1)
                        } else {
                            d3.select(this).style("opacity", 0.7).attr("stroke-width", 0)
                        }
                    }
                }
            }
    ...
    function actionPictogramView() {
        const ap_width = 300, ap_height = 320, ap_margin = 20;
        ...
        const jointStyles = {
            fill: agentColorScale(curren_main_agent.value),
            stroke: "black",
            "stroke-width": 2,
            "fill-opacity": 0.5,
            "stroke-opacity": 1
        ...
        const head_pos = {x: ap_width/2, y: ap_margin + 20}
        const joint_r = 10
        const joint_data = [
            ...
            {name: 'right_foot', x: head_pos.x - 50, y: head_pos.y + 265, r: 15},
            {name: 'left_foot', x: head_pos.x + 50, y: head_pos.y + 265, r: 15},
        ...
        const body_g = ap_svg.append("g")
        body_g.selectAll("circle")
            .data(joint_data).enter()
            .append("circle")
            .attr("class", d => d.name)
            .styles(jointStyles)
            .attrs(circleData)
